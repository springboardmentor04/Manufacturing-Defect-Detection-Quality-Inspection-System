...
// 15-minute inactivity rule: A production line is 'Running' if scanned within last 15 min, otherwise 'Idle'
export const LINE_IDLE_TIMEOUT_MS = 15 * 60 * 1000;

export function getLineOperatingStatus(lineName, liveBatches = [], nowMs = Date.now()) {
  const lineBatches = (liveBatches || []).filter(b => b.line === lineName);
  let latestCapturedMs = 0;
  for (const b of lineBatches) {
    if (b.captured) {
      const t = new Date(b.captured).getTime();
      if (!isNaN(t) && t > latestCapturedMs) {
        latestCapturedMs = t;
      }
    }
  }
  const isRunning = latestCapturedMs > 0 && (nowMs - latestCapturedMs <= LINE_IDLE_TIMEOUT_MS);
  const idleMinutes = latestCapturedMs > 0 ? Math.max(0, Math.floor((nowMs - latestCapturedMs) / 60000)) : null;
  return {
    isRunning,
...
export const productionLines = [
  { id: "L01", name: "Line 01", product: "Hazelnut Cap", status: "Idle", oee: 0, unitsToday: 0, yield: 0, shift: "—", downtime: 480 },
  { id: "L02", name: "Line 02", product: "Bracket Assembly", status: "Idle", oee: 0, unitsToday: 0, yield: 0, shift: "—", downtime: 480 },
  { id: "L03", name: "Line 03", product: "Seal Ring", status: "Idle", oee: 0, unitsToday: 0, yield: 0, shift: "—", downtime: 480 },
  { id: "L04", name: "Line 04", product: "Motor Housing", status: "Idle", oee: 0, unitsToday: 0, yield: 0, shift: "—", downtime: 480 },
];
...
export const productionAlerts = [
  { id: "A01", severity: "high", line: "Line 04", category: "Defect Threshold", message: "Defect rate exceeds 5% threshold (6.2% observed)", time: "14 min ago", acknowledged: false },
  { id: "A02", severity: "medium", line: "Line 01", category: "OEE Warning", message: "OEE dropped below 85% target (current: 61.5%)", time: "38 min ago", acknowledged: false },
  { id: "A03", severity: "high", line: "Line 02", category: "AI Health", message: "Vision camera inference confidence dip (<90% on Batch #B104)", time: "52 min ago", acknowledged: false },
  { id: "A04", severity: "low", line: "Line 02", category: "Maintenance", message: "Scheduled tooling maintenance window in 2 hours", time: "1h ago", acknowledged: true },
  { id: "A05", severity: "high", line: "Line 03", category: "Line Stoppage", message: "Line idle — continuous standby exceeded 4 hours", time: "2h ago", acknowledged: true },
  { id: "A06", severity: "medium", line: "Line 01", category: "Telemetry", message: "Conveyor speed variance detected (±12% deviation from standard)", time: "3h ago", acknowledged: true },
  { id: "A07", severity: "low", line: "Line 04", category: "Shift Sync", message: "Batch inspection handoff sync completed successfully", time: "4h ago", acknowledged: true },
  { id: "A08", severity: "medium", line: "Line 01", category: "Defect Spike", message: "High concentration of Surface Scratches detected on Line 01", time: "5h ago", acknowledged: true },
];
