...
export function DetectionPreview({ data }: { data: Inspection }) {
  const imageUrl = data.image_path?.startsWith('http') 
    ? data.image_path 
    : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${data.image_path}`;
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
        ...
        <span className="text-xs font-mono text-slate-500">{data.inspection_id}</span>
      </div>
      
      <div className="relative aspect-square bg-slate-950 flex items-center justify-center p-4">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={imageUrl} alt={data.dataset_category} className="max-w-full max-h-full rounded-lg object-contain shadow-xl border border-slate-800" />
      ...
      <div className="p-6 space-y-4 flex-1 flex flex-col">
        <div className="flex justify-between items-start">
          <div>
            <h4 className="font-bold text-lg text-slate-900 dark:text-white capitalize">{data.dataset_category.replace('_', ' ')}</h4>
          ...
          <div className={`px-3 py-1 rounded-lg font-bold text-lg border ${
            ...
            {data.inspection_result || data.status}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
            ...
            <p className="font-mono font-bold text-slate-900 dark:text-white">{data.confidence ? `${data.confidence}%` : 'N/A'}</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg">
          ...
          <span>Upload Time: <strong className="text-slate-700 dark:text-slate-300">{new Date(data.upload_time).toLocaleString()}</strong></span>
        </div>
