...
def initialize_models():
    ...
    # Check for YOLO weights (best.pt or last.pt)
    yolo_best = WEIGHTS_DIR / "best.pt"
    yolo_last = WEIGHTS_DIR / "last.pt"
    target_yolo = yolo_best if yolo_best.exists() else (yolo_last if yolo_last.exists() else None)

    if target_yolo:
        try:
            import ultralytics
            model_manager.yolo_model = ultralytics.YOLO(str(target_yolo))
            model_manager.yolo_loaded = True
            print(f"[YOLO Models] Successfully loaded YOLO model from {target_yolo.name}")
        except ImportError:
            print(f"[YOLO Models] Found {target_yolo.name} in {WEIGHTS_DIR}. (Tip: Install 'ultralytics' package via `pip install ultralytics` to enable native YOLO inference)")
        except Exception as e:
            print(f"[YOLO Models] Warning: Failed to load {target_yolo.name}: {e}")
    else:
        print(f"[YOLO Models] Note: Place 'best.pt' or 'last.pt' in {WEIGHTS_DIR} for YOLO defect detection.")

def get_classifier():
    return model_manager.classifier

def get_segmenter():
    return model_manager.segmenter

def get_yolo_model():
    return model_manager.yolo_model

def get_model():
    return model_manager.yolo_model

def get_grad_cam():
    return model_manager.grad_cam

def is_model_loaded():
    return model_manager.yolo_loaded or model_manager.classifier_loaded
