...
def load_model():
    ...
    if not MODEL_PATH.exists():
        ...
        raise FileNotFoundError(
            f"\nPill model not found:\n"
            f"{MODEL_PATH}\n"
        )

    print(
        "Loading Pill ResNet18..."
    )
    ...
    if "fc.1.weight" not in state_dict:
        ...
        raise RuntimeError(
            "\nPill checkpoint architecture mismatch.\n"
            "Expected fc.1.weight."
        )

    classifier_shape = tuple(
        state_dict[
            "fc.1.weight"
        ].shape
    )
    ...
    if classifier_shape != expected_shape:
        ...
        raise RuntimeError(
            "\nPill classifier shape mismatch.\n"
            f"Expected: {expected_shape}\n"
            f"Found   : {classifier_shape}"
        )
    ...
    if missing or unexpected:
        ...
        raise RuntimeError(
            ...
            "match the expected Pill architecture."
        )
