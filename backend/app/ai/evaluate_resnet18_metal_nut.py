"""
VisionInspect AI
Metal Nut ResNet18 Evaluation

Matches the generic train_resnet18_category.py architecture.
"""
...
def build_model():
    ...
    # Generic trainer classifier:
    #
    # fc.0 = Dropout
    # fc.1 = Linear
    #
    # This matches the checkpoint produced by
    # train_resnet18_category.py.
...
def load_model():
    ...
    if not MODEL_PATH.exists():
        ...
        raise FileNotFoundError(
            f"\nMetal Nut model not found:\n"
            f"{MODEL_PATH}\n\n"
            f"Train the metal_nut model first."
        )

    print(
        "Loading Metal Nut ResNet18..."
    )
    ...
    if "fc.1.weight" not in state_dict:
        ...
        raise RuntimeError(
            "\nERROR: Metal Nut checkpoint does not "
            "contain expected fc.1.weight.\n\n"
            "Expected generic trainer architecture:\n"
            "  fc.0 = Dropout\n"
            "  fc.1 = Linear(512, 5)"
        )
    ...
    if actual_shape != expected_weight_shape:

        raise RuntimeError(
            "\nERROR: Metal Nut classifier shape mismatch.\n"
    ...
    if missing or unexpected:
        ...
        raise RuntimeError(
            "\nERROR: Checkpoint does not exactly "
            "match the generic trainer architecture."
        )
...
def main():
    ...
    if errors:
        ...
        for error in errors:
            ...
            print(
                f"{error['filename']:<15}"
                f"Actual={error['actual']:<20}"
                f"Predicted={error['predicted']:<20}"
                f"Confidence="
    ...
    print("=" * 90)
    print(
        "FINAL METAL_NUT BACKEND VERDICT"
    )
    print("=" * 90)
